 ...
 * @author santi
 */
public class HEIRUCTUnitActions extends AIWithComputationBudget implements InterruptibleAI {
    ...
    HEIRUCTUnitActionsNode tree = null;
    ...
    public HEIRUCTUnitActions(UnitTypeTable utt) {
        this(100,-1,100,10,
             new RandomBiasedAI(),
    ...
    public HEIRUCTUnitActions(int available_time, int available_playouts, int lookahead, int max_depth, AI policy, EvaluationFunction a_ef) {
    ...
    public AI clone() {
        return new HEIRUCTUnitActions(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, randomAI, ef);
    }  
    ...
    public void startNewComputation(int a_player, GameState gs) {
        ...
        tree = new HEIRUCTUnitActionsNode(playerForThisComputation, 1-playerForThisComputation, gs, null, evaluation_bound);
        gs_to_start_from = gs;
...
//        long cutOffTime = (TIME_BUDGET>0 ? start + TIME_BUDGET:0);
        ...
        while(true) {
            HEIRUCTUnitActionsNode leaf = tree.UCTSelectLeaf(playerForThisComputation, 1-playerForThisComputation, MAX_TREE_DEPTH);
...
//                System.out.println(evaluation_bound + " -> " + evaluation + " -> " + (evaluation+evaluation_bound)/(evaluation_bound*2));
    ...
    public PlayerAction getMostVisited(HEIRUCTUnitActionsNode current, int time) {
        ...
        for(int i = 0;i<current.children.size();i++) {
            HEIRUCTUnitActionsNode child = current.children.get(i);
