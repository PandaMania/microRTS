 ...
 * @author santi
 */
public class HEIRUCTNodeFirstPlayUrgency {
...
//    static float C = 1;   // this is the constant that regulates exploration vs exploitation, it must be tuned for each domain
    ...
    HEIRUCTNodeFirstPlayUrgency parent = null;
    public GameState gs;
    ...
    HashMap<Long,HEIRUCTNodeFirstPlayUrgency> childrenMap = new LinkedHashMap<Long,HEIRUCTNodeFirstPlayUrgency>();    // associates action codes with children
    public List<HEIRUCTNodeFirstPlayUrgency> children = null;
    ...
    public HEIRUCTNodeFirstPlayUrgency(int maxplayer, int minplayer, GameState a_gs, HEIRUCTNodeFirstPlayUrgency a_parent, float bound, double a_FPUValue) throws Exception {
        parent = a_parent;
        gs = a_gs;
...
//            actions = gs.getPlayerActions(maxplayer);
            ...
            children = new ArrayList<HEIRUCTNodeFirstPlayUrgency>();
...
//            actions = gs.getPlayerActions(minplayer);
    ...
    public HEIRUCTNodeFirstPlayUrgency UCTSelectLeaf(int maxplayer, int minplayer, long cutOffTime, int max_depth) throws Exception {
        ...
        // Bandit policy:
        double best_score = 0;
        HEIRUCTNodeFirstPlayUrgency best = null;
        if (DEBUG>=1) System.out.println("HEIRUCTNodeFirstPlayUrgency.UCTSelectLeaf:");
        for(int i = 0;i<children.size();i++) {
            HEIRUCTNodeFirstPlayUrgency child = children.get(i);
        ...
        if (moveGenerator!=null) {
            ...
            if (attemptsLeft>0) {
                ...
                HEIRUCTNodeFirstPlayUrgency node = new HEIRUCTNodeFirstPlayUrgency(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, FPUvalue);
                children.add(node);
    ...
    public double childValue(HEIRUCTNodeFirstPlayUrgency child) {
    ...
    public void showNode(int depth, int maxdepth) {
        ...
        HEIRUCTNodeFirstPlayUrgency mostVisited = null;
        for(int i = 0;i<children.size();i++) {
            HEIRUCTNodeFirstPlayUrgency child = children.get(i);
