 ...
 * @author santi
 */
public class HEIRDownsamplingUCTNode {
...
//    static float C = 5;   // this is the constant that regulates exploration vs exploitation, it must be tuned for each domain
    ...
    HEIRDownsamplingUCTNode parent = null;
    public GameState gs;
    ...
    public List<HEIRDownsamplingUCTNode> children = null;
    float evaluation_bound = 0;
    float accum_evaluation = 0;
    int visit_count = 0;
    
    public HEIRDownsamplingUCTNode(int maxplayer, int minplayer, GameState a_gs, HEIRDownsamplingUCTNode a_parent, long MAXACTIONS, float bound) throws Exception {
        parent = a_parent;
        gs = a_gs;
    ...
    public HEIRDownsamplingUCTNode UCTSelectLeaf(int maxplayer, int minplayer, long MAXACTIONS, long cutOffTime, int max_depth) throws Exception {
        ...
        if (moveGenerator!=null && actions==null) {
            ...
            children = new ArrayList<HEIRDownsamplingUCTNode>();
        ...
        if (hasMoreActions) {
            ...
            if (children.size()>=actions.size()) {
                ...
                HEIRDownsamplingUCTNode node = new HEIRDownsamplingUCTNode(maxplayer, minplayer, gs2.clone(), this, MAXACTIONS, evaluation_bound);
                children.add(node);
                return node;                
            }            
        }
        ...
        // Bandit policy:
        double best_score = 0;
        HEIRDownsamplingUCTNode best = null;
        for(int i = 0;i<children.size();i++) {
            HEIRDownsamplingUCTNode child = children.get(i);
